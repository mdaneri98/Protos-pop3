...
main(const int argc, char **argv) {
    ...
    */
    ...
    log(LOG_DEBUG, "Registering IPv4 server fd")
    if (SELECTOR_SUCCESS != selector_register(selector, server, &server_handler, OP_READ, args)) {
        err_msg = "unable to register server fd for IPv4 socket.";
        goto finally;
    }

    log(LOG_DEBUG, "Registering IPv6 server fd")
    if (SELECTOR_SUCCESS != selector_register(selector, server_6, &server_handler, OP_READ, args)) {
        err_msg = "unable to register server fd for IPv4 socket.";
        goto finally;
    }
